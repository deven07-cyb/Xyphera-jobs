...
const handleSubmit = async (e) => {
  ...
  if (
    ...
    setError(
      'Please fill in all required fields: Job Title, Location, Description, Job Type, Work Environment, Experience Level'
    );
    setIsSubmitting(false);
    return;
  }
  ...
  const skills = formData.skillsRequired
    ? formData.skillsRequired.split(',').map((skill) => {
        ...
        return {
          ...
          skill_category: skillName.includes('Research') ? 'soft' : 'technical',
  ...
  formDataToSend.append('description', formData.shortDescription);
  ...
  formDataToSend.append('employment_type', formData.jobType.toLowerCase());
  formDataToSend.append('experience_level', formData.experienceRequired.toLowerCase());
  formDataToSend.append('education_level', formData.educationLevel);
  formDataToSend.append('salary_min', salary_min);
  formDataToSend.append('salary_max', salary_max);
  formDataToSend.append('salary_currency', formData.salaryCurrency);
  formDataToSend.append('salary_type', formData.salaryType);
  formDataToSend.append('remote_type',
    formData.workEnvironment.toLowerCase() === 'remote'
      ? 'fully_remote'
      : formData.workEnvironment.toLowerCase()
  );
  formDataToSend.append('priority', formData.priority);
  formDataToSend.append('category_id', formData.categoryId);
  formDataToSend.append('application_method', formData.applicationMethod);
  formDataToSend.append('auto_close_after_deadline', formData.autoCloseAfterDeadline);
  formDataToSend.append('show_salary', formData.showSalary);
  formDataToSend.append('status', formData.status);
  formDataToSend.append('responsibilities', formData.responsibilities || '');
  formDataToSend.append('benefits', formData.benefits || '');

  // If skills must be sent as JSON array in FormData
  if (skills.length > 0) {
    formDataToSend.append('skills', JSON.stringify(skills));
  }

  const apiUrl = 'http://127.0.0.1:5000/api/v1/jobs/';
  try {
    const response = await fetch(apiUrl, {
      method: 'POST',
      headers: {
  ...
  return (
    <div className="xyj-homepage-wrapper" id="xyj-post-job">
      ...
      <div className="xyj-job-posting-container">
        <div className="xyj-header-section">
          <Link to="/employerhomepage" aria-label="Back to home" style={{alignItems:"center", display:"flex"}}>
              <img src="/imageswebsite/arrowleft.png" alt="Back arrow" className="xyj-back-arrow" />
              <span style={{ color: '#000000' }} className="xyj-back-text">
                Back
              </span>
          ...
          <div className="xyj-form-section">
            ...
            <div className="xyj-form-row">
              ...
              <input
                type="text"
                name="responsibilities"
                placeholder="Responsibilities (e.g., Develop software, QA)"
                value={formData.responsibilities}
                onChange={handleInputChange}
                required
                aria-label="Location"
